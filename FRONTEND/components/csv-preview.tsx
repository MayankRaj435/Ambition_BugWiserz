      ...
      <CardContent className="space-y-4">
                ...
                <tr key={rIdx} className={rIdx % 2 === 0 ? "bg-background" : "bg-muted/30"}>
                  {filtered.headers.map((_, cIdx) => {
                    ...
                    return (
                      <td
                        key={cIdx}
                        className={`px-3 py-2 border-b border-border ${highlighted ? "bg-primary/20" : ""}`}
                      >
                        {cell}
                      </td>
                    )
        ...
        {variant === "full" && (
          <div className="flex items-center justify-between pt-2">
            <Link href="/" className="text-sm underline underline-offset-4 text-primary hover:text-primary/80">
              ← Back to Upload
            </Link>
