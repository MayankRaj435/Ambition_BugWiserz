...
export default function HomePage() {
  ...
  // Reset UI state on initial load or navigation to HomePage, keeping only preview and previous datasets
  // Reset UI state on initial load or navigation to HomePage
  useEffect(() => {
    setFileName(null)
    setTarget("")
    setSplit(80)
    ...
    setTaskId(null)
    setTaskType(null)
    setDataset(null) // Explicitly clear dataset on mount/refresh

    // We also want to clear the storage so it doesn't persist
    localStorage.removeItem(DATASET_STORAGE_KEY)

    // Restore previous list for sidebar (we keep this history)
    const raw = localStorage.getItem(INDEX_KEY)
    const arr = raw ? JSON.parse(raw) : []
    setPrevList(Array.isArray(arr) ? arr.sort((a, b) => b.savedAt - a.savedAt) : [])
  }, [router]) // runs every mount/navigation to HomePage

  // Helper for saving a new dataset to index/storage
  ...
  const onFiles = useCallback(
    async (files: FileList | null) => {
      ...
      try {
        ...
        setTaskType(determineTaskType(ds, target))
  ...
  // Model options (doesn't change)
  const modelsList = useMemo(
    () => [
      { name: "AutoML (Best Model)", for: "any" },
    ...
    try {
      const res = await fetch("http://localhost:5000/train", {
        ...
        body: JSON.stringify({
          ...
          models: models.map(m => m === "AutoML (Best Model)" ? "auto" : m),
        }),
      })
      const data = await res.json()
      ...
      toast({ title: "Training started", description: "Models are training in the background..." })

      // Poll for results
      const intervalId = setInterval(async () => {
        try {
          const pollRes = await fetch(`http://localhost:5000/results/${taskId}`)
          ...
          if (pollData.status === "completed") {
            ...
            toast({
              ...
              description: `Metric: ${pollData.metric} · ${pollData.results.length} model(s) evaluated.`,
            })
            router.push(`/results/${taskId}`)
          } else if (pollData.status === "failed") {
            clearInterval(intervalId)
            setIsTraining(false)
            toast({ title: "Training failed", description: pollData.error || "Unknown error" })
          }
        } catch (e) {
      ...
      toast({ title: "Error", description: "Failed to start training." })
      setIsTraining(false)
    }
  }
              ...
              <>
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {modelsList.map(({ name, for: applicableFor }) => {
                        ...
                        const disabled = taskType ? (applicableFor !== taskType && applicableFor !== "any") : false
                        return (
                          <button
                            key={name}
                            type="button"
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button type="button" onClick={handleTrain} disabled={!canTrain || isTraining}>
